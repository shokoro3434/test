...
public class TargetResultCleaner {
	public static void main(String[] args) throws Exception{
		new TargetResultCleaner().perform(args);
	}
	private List<String> toList(CSVRecord rec){
		...
		list.add(rec.get(4).trim());//num
		list.add(rec.get(91).trim());//interval
		list.add(rec.get(22).trim());//time
...
//		}
	...
	private void perform(String [] args) throws IOException{
		...
		try{
			...
			//header
			writer.writeNext(new String[] { "num", "interval","tm" });
